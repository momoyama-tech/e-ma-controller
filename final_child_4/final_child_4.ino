...
const int touchPins[] = {T5, T6, T7, T9};  // 子機タッチピン
const int ledPins[] = {18, 19, 21, 22};    // LEDピン
...
const int threshold[] = {10, 10, 10, 7};// タッチしきい値
...
void updateTouchState(int index, int touchedValue) {
  sendData[index + dataLength - 28] = (touchedValue < threshold[index]) ? 1 : 0; // 後ろ6つに子機のタッチ情報を格納
}
...
void updateLedBrightness() {
  for (int i = 0; i < numLeds; i++) {
    ...
    ledcWrite(ledPins[i], intensity[i + dataLength - 28] + sendData[i + dataLength - 28]*5  );  // LED強度を設定 
  }
}
